<script>
</script>

<div class="w-full h-50 fixed bg-dark-beige shadow-md flex justify-between top-0">
  <a href="/">
    <img alt="Nihogami Logo" class="max-h-full ml-[10px]" src="static/logo.png" />
  </a>
  <div class="mr-[10px] flex items-center justify-between align-middle">
    <a class="text-base mx-2 hover:underline hover:font-bold hover:text-dark-red" href="/">Home</a>
    <a class="text-base mx-2 hover:underline hover:font-bold hover:text-dark-red" href="/cardGame">Vocab Cards</a>
  </div>
</div>
